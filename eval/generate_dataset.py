...
GENERATE_PROMPT = """
You are an expert curriculum designer. Given the following academic text excerpt,
generate ONE high-quality question and its correct ground-truth answer. 
The question should test the student's understanding of the text.

Text Excerpt:
{text}

Respond ONLY with a valid JSON object containing "query" (the question)
and "ground_truth_answer" (the correct answer based on the text).
Example: {{"query": "What is the primary function of TCP?",
"ground_truth_answer": "TCP guarantees reliable, ordered delivery of packets."}}
"""
...
def extract_chunks(processed_dir: Path, max_chunks: int = 60) -> list[dict]:
    ...
    for md_file in processed_dir.rglob("*.md"):
        ...
        for p in paragraphs:
            chunks.append({"text": p, "course_code": course_code, "source_file": md_file.name})
    random.shuffle(chunks)
    return chunks[:max_chunks]


async def generate_dataset(
    processed_dir_path: str = "artifacts/data/processed", output_file: str = "eval/dataset.json", num_queries: int = 60
):
