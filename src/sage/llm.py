    ...
    if nvidia_smi is not None:
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                log.warning("nvidia_smi_no_data", returncode=result.returncode, stderr=result.stderr.strip()[:300])
        except (subprocess.TimeoutExpired, ValueError, OSError) as exc:
            log.warning("gpu_detection_cuda_failed", error=str(exc))
    else:
        log.info("nvidia_smi_not_found", hint="No NVIDIA GPU or drivers not installed; running CPU-only.")
...
def _resolve_context_size(effective_backend: str, vram_mb: int, cfg: LLMSettings) -> int:
    ...
    if effective_backend == "cpu":
        ...
        log.info("context_size_resolution", available_ram_mb=available_mb, model_size_mb=model_size_mb, backend="cpu")
...
def _wait_for_server(
    ...
    while time.monotonic() < deadline:
        ...
        if now - last_progress_log >= 10.0:
            ...
            log.info("llama_server_loading", elapsed_s=round(elapsed), timeout_s=round(timeout_s), port=port)
...
def start_llm_server() -> tuple[subprocess.Popen[bytes], int, dict[str, Any]]:
    ...
    log.info("llama_server_ready", backend=effective_backend, ctx_size=ctx_size, gpu_layers=gpu_layers, port=port)
...
def start_utility_server() -> tuple[subprocess.Popen[bytes], int]:
    ...
    if not util_model.exists():
        log.warning(
            ...
            path=str(util_model),
            hint="Utility model not available. Memory extraction will fall back to the primary model.",
        )
        raise FileNotFoundError(
    ...
    if not _binary_installation_ok(cpu_bin, backend="cpu"):
        raise FileNotFoundError(
            f"CPU llama-server needed for utility model, but installation incomplete at: {cpu_bin.parent}"
        )
