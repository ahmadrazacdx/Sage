...
async def _digest_subtopic(
    ...
    try:
        ...
        log.info(
            "digest_ok", subtopic=subtopic.name[:40], words=len(digest.split()), ctx=budget.ctx_size
        )
        return digest.strip()
    except Exception as exc:
...
async def research_node(
    ...
    *,
    ...
    for attempt in range(1, _MAX_PLAN_RETRIES + 1):
        try:
            ...
            if len(plan.subtopics) > budget.max_subtopics:
                ...
                log.info(
                    "plan_subtopics_capped", cap=budget.max_subtopics, original=len(plan.subtopics)
                )
            log.info(
    ...
    for subtopic in plan.subtopics:
        ...
        if q.web:
            tagged.append(
                (subtopic.name, _search_source(search_web, q.web, "web", search_cfg.web_timeout))
            )
        if q.encyclopedia:
            tagged.append(
                (
    ...
    try:
        ...
        report = (
            report_result.content if isinstance(report_result, AIMessage) else str(report_result)
        )
    ...
    for review_iter in range(1, max_iters + 1):
        ...
        if report_tokens + review_overhead > budget.ctx_size - 300:
            ...
            log.warning(
                "review_report_trimmed", original_tokens=report_tokens, keep_chars=keep_chars
            )
        else:
        ...
        try:
            review_timeout = min(
                float(timeout), 90.0
            )  # cap so a slow reviewer can't double overall latency
            ...
            report_result = await asyncio.wait_for(
                report_chain.ainvoke(
                    {
                        "sources": _trim(
                            revision_context, budget.source_char_budget, ellipsis=False
                        ),
                        "title": plan.title,
                        "source_references": source_refs,
                    }
