import pytest
from sage.agents.response import response_node, _build_citation_map, _rewrite_citations, _sanitise_claim, _build_references_section

@pytest.mark.asyncio
async def test_response_node_basic():
    state = {
        "response": "Hello [KU1]",
        "knowledge_units": [{"id": "KU1", "claim": "Fact 1", "source_file": "doc.pdf"}]
    }
...
def test_response_helpers():
    ...
    refs = _build_references_section([
        {"id": "KU1", "source_file": "S1", "claim": "C1", "source_page": "10", "confidence": "high"}
    ], {"KU1": 1})
