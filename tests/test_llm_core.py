...
def test_detect_gpu_cuda():
    with patch("shutil.which", return_value="/usr/bin/nvidia-smi"), patch("subprocess.run") as mock_run:
...
def test_kill_orphaned_servers():
    ...
    with patch("psutil.process_iter", return_value=[mock_proc1, mock_proc2]), patch("time.sleep"):
...
def test_wait_for_server_timeout():
    ...
    mock_proc.poll.return_value = None
    with (
        patch("urllib.request.urlopen", side_effect=urllib.error.URLError("test")),
        patch("time.monotonic", side_effect=[0, 0.5, 1.5, 2.0]),
        patch("time.sleep"),
        pytest.raises(RuntimeError, match="llama-server did not become healthy"),
    ):
