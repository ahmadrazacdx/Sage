...
async def test_extract_memories():
    ...
    mems = await memory.extract_memories(
        "My name is Alex. I am in Data Structures.",
        "Hello Alex! I am happy to help you with Data Structures today.",
        llm,
    )
    assert len(mems) == 2
    ...
    mems_none = await memory.extract_memories(
        "User says something longer.", "Assistant says something longer too.", llm
    )
    assert len(mems_none) == 0

    llm_failing = MockFailingLLM()
    mems_exc = await memory.extract_memories(
        "User says something longer.", "Assistant says something longer too.", llm_failing
    )
...
async def test_deduplicate_and_store():
    memories = [{"category": "identity", "content": "Name is Alex"}, {"category": "study", "content": "Loves Python"}]
...
async def test_post_turn_memory_hook():
    ...
    await memory.post_turn_memory_hook(
        "I am learning how to test Python code today.", "That is a great skill to have. What framework?", llm
    )
