...
from sage.network import NetworkMonitor, check_internet


@pytest.fixture
def dummy_network_settings():
    return NetworkSettings(force_offline=False, check_interval=5, timeout=0.5)
...
async def test_network_monitor_poll(dummy_network_settings):
    ...
    with patch("sage.network.check_internet", side_effect=mock_check):
        await monitor.start()
