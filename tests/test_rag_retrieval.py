...
from sage.rag.retrieval import (  # noqa: E402
    _dense_retrieve,
    _embed_query,
    _get_embed_model,
    _hydrate_chunks,
...
def test_load_bm25_index(tmp_path):
    with patch("sage.rag.retrieval.get_settings") as mock_settings:
        ...
        mock_bm25 = DummyBM25()
        data = {"bm25": mock_bm25, "ids": ["id1", "id2"], "metadatas": [{"course_code": "CS101"}, {}]}
...
def test_dense_retrieve():
    with (
        patch("sage.rag.retrieval.get_curriculum_collection") as mock_get_coll,
...
def test_hydrate_chunks():
    ...
    with patch("sage.rag.retrieval.get_curriculum_collection") as mock_get_coll:
        mock_coll = MagicMock()
        mock_coll.get.return_value = {"ids": ["id2"], "metadatas": [{"course_code": "CS102"}], "documents": ["doc2"]}
        mock_get_coll.return_value = mock_coll
...
async def test_hybrid_retrieve():
    with (
        patch("sage.rag.retrieval.get_settings") as mock_settings,
