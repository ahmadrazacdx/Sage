...
from sage.agents.research import ResearchPlan, ResearchReview, Subtopic, SubtopicQueries, research_node
...
async def test_research_node_success():
    ...
    plan = ResearchPlan(
        ...
        subtopics=[
            Subtopic(
                name="Basics",
                queries=SubtopicQueries(academic="AI", web="AI basics", encyclopedia="Artificial Intelligence"),
            )
        ],
    )
    ...
    with (
        patch("sage.agents.research.ainvoke_structured_with_fallback", side_effect=[plan, review_pass]),
...
async def test_research_node_fallback_plan():
    ...
    with (
        patch(
            "sage.agents.research.ainvoke_structured_with_fallback",
            side_effect=[Exception("Fail"), Exception("Fail"), review_pass],
        ),
        patch("sage.tools.search.search_arxiv") as m_arxiv,
        patch("sage.tools.search.search_web") as m_web,
        patch("sage.tools.search.search_wikipedia") as m_wiki,
        patch("sage.agents.research._run_digest_phase", new_callable=AsyncMock) as m_digest,
        patch("sage.agents.research.ChatOpenAI.ainvoke", new_callable=AsyncMock),
    ):
...
async def test_research_node_review_loop():
    ...
    with (
        patch("sage.agents.research.ainvoke_structured_with_fallback", side_effect=[plan, review_revise, review_pass]),
        patch("sage.tools.search.search_web") as m_web,
        patch("sage.agents.research._run_digest_phase", new_callable=AsyncMock) as m_digest,
        patch("sage.agents.research.export_markdown"),
        patch("sage.agents.research.export_pdf"),
    ):
        m_web.ainvoke = AsyncMock(return_value={"results": []})
        m_digest.return_value = "Digest content"

        llm.ainvoke = AsyncMock(side_effect=[AIMessage(content="Report Draft 1"), AIMessage(content="Report Draft 2")])
...
async def test_research_node_writer_fails():
    ...
    with (
        patch("sage.agents.research.ainvoke_structured_with_fallback", return_value=plan),
        patch("sage.agents.research._run_digest_phase", new_callable=AsyncMock) as m_digest,
    ):
...
async def test_digest_subtopic():
    from sage.agents.research import ContextBudget, _digest_subtopic
...
def test_research_helpers():
    from sage.agents.research import (
        _all_refs_look_fake,
        _force_replace_references,
        _normalize_references_section,
        _trim,
    )
