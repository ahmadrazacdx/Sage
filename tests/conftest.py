...
from typing import Any, AsyncIterator, Iterator, Optional, List, Dict
import asyncio
...
def mock_settings(monkeypatch):
    from sage.config import get_settings, Settings, DatabaseSettings, LLMSettings
...
class ExactMockChatOpenAI(ChatOpenAI):
    ...
    async def ainvoke(self, input: Any, config: Optional[dict] = None, **kwargs: Any) -> BaseMessage:
        return self.invoke(input, config, **kwargs)

    def stream(self, input: Any, config: Optional[dict] = None, **kwargs: Any) -> Iterator[BaseMessage]:
        yield AIMessage(content=self.mock_response, tool_calls=self.mock_tool_calls)

    async def astream(self, input: Any, config: Optional[dict] = None, **kwargs: Any) -> AsyncIterator[BaseMessage]:
        yield AIMessage(content=self.mock_response, tool_calls=self.mock_tool_calls)
...
def patch_llm_factory(monkeypatch):
    ...
    monkeypatch.setattr(llm, "start_llm_server", lambda: (MagicMock(), 8080, {"backend": "cpu", "vram_mb": 0, "gpu_name": None}))
